...
import { getNationalities, getReferralTypes, addCommunityOperationUser } from '@/lib/seed'
...
const formInputs: IFormstep[] = [
    ...
    { name: "referral_type", label: "Tipo Referimiento", type: "select", validation: { required: "Campo obligatorio" } },
...
const formSchema = z.object({
    ...
    referral_type: z.string({ required_error: "El tipo de referencia es requerido" }),
...
const AddUserForm = ({ params }: TCommunityOperativeUserParams) => {
        ...
        async function loadData() {
            try {
                ...
                if (error instanceof Error) {
                    toast({
                        variant: "destructive",
                        title: "Agregar Usuario A Operativo Comunidad || " + appName,
    ...
    const onSubmit: SubmitHandler<IFormInput> = async (data) => {
        ...
        const newData = {
            ...data,
            referral_type_id: data.referral_type,
            user_id: Number(session?.user.id),
            sai_id: Number(session?.user.id_sai),
            from_username: session?.user.username,
            community_operation_id: Number(params.id)
        }

        const { error, data: resData, message } = await addCommunityOperationUser(Array(newData))
